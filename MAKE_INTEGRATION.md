...
4. **データの保存ロジック**
   - **常に新しいレコードとして保存されます**
   - 同じ `orderId` でも、毎回新しい診断結果として別レコードに保存されます
   - これにより、同じオーダー番号でも異なる診断結果を複数回取得できます
   - Make側で使用可否を判断した後、Supabaseには常に新規レコードとして保存してください

5. **フロントエンドのポーリング**
   - フロントエンドは3秒ごとにSupabaseをポーリングして結果を取得しています
   - ポーリング時には `userId` と `orderId` の両方を使って最新のレポートを取得します
